...
class _RegisterPageState extends State<RegisterPage> {
  ...
  Future<void> _onRegister() async {
    ...
    if (name.isEmpty || email.isEmpty || password.isEmpty) {
      _snack('Semua field wajib diisi', error: true);
      return;
    }
    if (password != confirm) {
    ...
    if (password.length < 8) {
      _snack('Password minimal 8 karakter', error: true);
      return;
    }
    ...
    if (res['status'] == 'success') {
      ...
      _snack(res['message'] ?? 'Registrasi gagal', error: true);
    }
  }
            ...
            child: Column(
              children: [
                CircleAvatar(
                  ...
                  child: const Icon(Icons.person_add_alt_1_outlined, color: AppColors.primary, size: 36),
                ),
                const SizedBox(height: 18),
                const Text("Create Account", style: TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                const SizedBox(height: 6),
                const Text("Start managing your inventory easily", style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: _input("Confirm Password", Icons.lock_outline,
                            suffix: IconButton(
                              icon: Icon(_showConfirm ? Icons.visibility_outlined : Icons.visibility_off_outlined, color: AppColors.textSecondary),
